from myhdl import *
import pytest
from mips.core.alu import def_alu
import random
...
def test_alu():
    ...
    op1, op2 = Signal(intbv(0, min=-2**32, max=2**32-1)), Signal(intbv(0, min=-2**32, max=2**32-1))
    ...
    sim = Simulation(alu_inst, tb_clk, tb_n)
    sim.run()
